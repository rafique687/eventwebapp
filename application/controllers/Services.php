...
class Services extends CI_Controller {
	...
	public function view_services()
	{
		...
		if(!isset($sessiontrue))
		...
		else
			{
                ...
                $or = array('service_status' => 2 );
				$data['all']=$this->adminlogin->allsetting("setting_table");	
				$data['product']=$this->Services_Model->viewservices("logistics",$condi,$or);
				//echo "<pre>"; print_r($data['product']);exit;
				$data['containt']='admin/view_logistics.php';
				$this->load->view("admin",$data);
			}
		}
		...
		public function addlogistics()
	{
		...
		{
			$data['all']=$this->adminlogin->allsetting("setting_table");
			$data['containt']="admin/add_logistics.php";
			$data['parent_cate']=$this->Product_Model->mainproduct("products");
			$this->load->view('admin.php',$data);
			//redirect("product/view_product?success=Product Successfully stored");
		}
		...
		if(!isset($sessiontrue))
			...
			$this->form_validation->set_rules('des', 'Product Description', 'required');
				...
				else
				{
					...
					$qry = $this->Product_Model->productinsert("logistics",$datta);
						if($qry)
						{
							redirect("Services/logistics?success=logistics Successfully Added");
						}
				}
			}
	}
	...
	{   //echo "yss";exit();
		...
		if(!isset($sessiontrue))
			...
			$condi=array("service_id "=>$_GET['id']);
			$data['editid']=$_GET['id'];
			$data['containt']="admin/edit_logistic.php";
			$data['product']=$this->Product_Model->itemrow('logistics',$condi);
			$this->load->view("admin",$data);
			}
	}
	...
	public function insert_editlogistic()
	{
		$sessiontrue=$this->session->userdata("user_name");
		if(!isset($sessiontrue))
			{
			...
			else
			{
				...
				$fire=$this->Product_Model->editinsert("logistics",$field,$condi);
				if($fire)
				{ 
					redirect("Services/logistics?success=Logistic Successfully Updated");
				}
			}
		}
	}
	...
	{  
		...
		if(!isset($sessiontrue))
			...
			else
			{
				$condi = array('service_id' => $_GET['id'] );
				$datt=array("service_status"=>0);
				$del=$this->Product_Model->deleteProduct('logistics',$condi,$datt);
				redirect("Services/logistics?success=Logistics Successfully Deleted");
			}
	}
